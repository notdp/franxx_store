...
export function GmailAccountManagement() {
  ...
  // iOS账号列表数据源
  const [iosAccounts] = useState([
    { id: '1', email: 'ios.user.001@icloud.com', fullName: 'John Smith', status: 'active' },
    { id: '2', email: 'ios.user.002@gmail.com', fullName: 'Sarah Johnson', status: 'suspended' },
    { id: '3', email: 'ios.user.003@outlook.com', fullName: 'Mike Chen', status: 'active' },
    { id: '4', email: 'ios.user.004@icloud.com', fullName: 'Emma Wilson', status: 'active' },
    { id: '5', email: 'ios.user.005@yahoo.com', fullName: 'David Lee', status: 'banned' }
  ]);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>管理所有Gmail账号，包括ChatGPT和Claude使用状态。iOS账号绑定需要从iOS账号管理界面进行设置。</CardDescription>
            </div>
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm">
                            {iosAccounts.find(ios => ios.id === account.bindedIOSAccount)?.email || account.bindedIOSAccount}
                          </span>
                          <span className="text-xs text-muted-foreground">
                            {iosAccounts.find(ios => ios.id === account.bindedIOSAccount)?.fullName || ''}
                          </span>
                        </div>
                      </div>
                    ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 bg-muted rounded-md border">
                {editingAccount?.bindedIOSAccount ? (
                  <div className="flex items-center space-x-2">
                    <Smartphone className="w-4 h-4 text-muted-foreground" />
                    <div className="flex flex-col">
                      <span className="text-sm">
                        {iosAccounts.find(ios => ios.id === editingAccount.bindedIOSAccount)?.email || editingAccount.bindedIOSAccount}
                      </span>
                      <span className="text-xs text-muted-foreground">
                        {iosAccounts.find(ios => ios.id === editingAccount.bindedIOSAccount)?.fullName || ''}
                      </span>
                    </div>
                  </div>
                ) : (
